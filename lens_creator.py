...
def gen_verts(seg, semi_dia, radius, z_offset=0):
    ...
    for i in range(-seg, seg+1):
        for j in range(-seg, seg+1):
            ...
            # the geometry is generated by
            # finding radius and angle of the vertices
            # (polar coordinate on x-y plane)

            r = max(abs(i), abs(j))  # radius ( in index 0, 1, 2... form )
            a = math.atan2(j, i)  # angle ( in radians )

            r_factor = min(1, r/seg/effective_factor)**2.5
            # r_factor 1: circle, 0: square shape
            # the power of 2.5 is given to smooth out the topology
            # maybe other number will do better but I just put 2.5 here for now.

            r = r*r_factor + (1-r_factor)*math.sqrt(i*i+j*j)
            r = r / seg * semi_dia  # radius ( the actual value )
